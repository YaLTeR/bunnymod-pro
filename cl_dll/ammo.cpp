...
// YaLTeR
int ammoConsumed;
int oldAmmoConsumed = 0;
float gaussboost_ammoConsumed;
int startAmmo = -1;
...
int CHudAmmo::Draw(float flTime)
{
	...
	if (m_pWeapon->iAmmoType > 0)
	{
		...
		else
		{
			...
			if (g_bGausscharge)
			{
				if (oldGausscharge != g_bGausscharge) // The gauss cannon started to charge
				{
					startAmmo = gWR.CountAmmo(pw->iAmmoType);
					oldAmmoConsumed = 0;
					timeTillNextAmmoConsume = 0.5;
					timeAmmoConsumed = gEngfuncs.GetClientTime();
				}

				ammoConsumed = startAmmo - gWR.CountAmmo(pw->iAmmoType);

				if (oldAmmoConsumed != ammoConsumed && oldAmmoConsumed > 0)
				{
					timeTillNextAmmoConsume = 0.3;
					timeAmmoConsumed = gEngfuncs.GetClientTime();
				}

				oldAmmoConsumed = ammoConsumed;

				if (ammoConsumed < 13)
					gaussboost_ammoConsumed = ammoConsumed + (1 - (((timeAmmoConsumed + timeTillNextAmmoConsume) - gEngfuncs.GetClientTime()) / timeTillNextAmmoConsume));
				else
					gaussboost_ammoConsumed = ammoConsumed;
			}

			oldGausscharge = g_bGausscharge;
		}
