...
extern cvar_t *cl_wallhack,*cl_modelalpha;
extern cvar_t *cl_rendermodels, *cl_rendergibs;
extern cl_entity_t *view;
void CStudioModelRenderer::StudioRenderModel( void )
{
	if ( !cl_rendermodels->value )
	{
		return;
	}
	
	if ( !cl_rendergibs->value )
	{
		if ( strstr( m_pCurrentEntity->model->name, "gib" ) != NULL )
		{
			return;
		}
	}
	...
	int rbis = m_pCurrentEntity->curstate.rendermode;

	if ((m_pCurrentEntity==view) && cl_modelalpha->value)
	{
		gEngfuncs.pTriAPI->RenderMode( kRenderTransAdd );
		gEngfuncs.pTriAPI->Brightness( 2 );
	}
	else
		gEngfuncs.pTriAPI->RenderMode( rbis );
