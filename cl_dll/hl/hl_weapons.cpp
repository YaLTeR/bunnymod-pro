...
void CBasePlayerWeapon::ItemPostFrame( void )
{
	...
	if ((m_pPlayer->pev->button & IN_ATTACK2) && (m_flNextSecondaryAttack <= 0.0)) // +attack2 is pressed && the weapon can do secondary attack already.
	{
	...
	else if ((m_pPlayer->pev->button & IN_ATTACK) && (m_flNextPrimaryAttack <= 0.0)) // +attack is pressed && the weapon can do primary attack already.
	{
...
void _DLLEXPORT HUD_PostRunCmd( struct local_state_s *from, struct local_state_s *to, struct usercmd_s *cmd, int runfuncs, double time, unsigned int random_seed )
{
	...
	//gEngfuncs.Con_Printf("HUD_PostRunCmd: onground = %i\n",g_onground);
