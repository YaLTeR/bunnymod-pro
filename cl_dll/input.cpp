...
void TAS_ConstructWishvel(const vec3_t &angles, float forwardmove, float sidemove, float upmove, float maxspeed, bool inDuck, vec3_t *wishvel)
{
	...
	if (CVAR_GET_FLOAT("tas_log") != 0)
	{
		...
		indenter->indent();
		gEngfuncs.Con_Printf("Forwardmove: %.8f; sidemove: %.8f; upmove: %.8f\n", forwardmove, sidemove, upmove);
		indenter->indent();
...
void TAS_ConstructWishvelWithButtons(const vec3_t &angles, const vec3_t &velocity, float velocityAngleFallback, float wishspeed, float upmove, float maxspeed, bool inDuck, bool onGround, vec3_t *wishvel)
{
	...
	bool rotatingLeft = (normangle(angles[1] - vel_angle) > 0);
	...
	newangles[1] = normangleengine(newangles[1] - (45 * buttons));

	if (CVAR_GET_FLOAT("tas_log") != 0)
	{
		indenter->startSection("TAS_ConstructWishvelWithButtons");
		indenter->indent();
	...
	double maxspeed, double accel, double maxvelocity, double wishspeed, double wishspeed_cap, double frametime, double pmove_friction, bool onGround, int waterlevel)
{
	...
	double distanceToPoint[2];
	for (int i = 0; i < 2; i++)
	{
		vec3_t newpos, viewangles, wishvel;
...
float TAS_Strafe(const vec3_t &viewangles, const vec3_t &velocity, const vec3_t &origin, double maxspeed, double accelerate, double airaccelerate, double maxvelocity, double wishspeed, double wishspeed_cap, double frametime, double pmove_friction, bool onGround, int waterlevel, float pitch)
{
	...
	switch (strafedir)
	{
		...
		case STRAFEDIR_POINT:
		{
			float point[2] = { CVAR_GET_FLOAT("tas_strafe_point_x"), CVAR_GET_FLOAT("tas_strafe_point_y") };

			float dirToPoint[2] = { point[0] - origin[0], point[1] - origin[1] };
			double velocityAngleFallback = atan2(dirToPoint[1], dirToPoint[0]) * M_RAD2DEG;
			if (speed == 0)
				vel_angle = velocityAngleFallback;
...
void TAS_DoStuff(const vec3_t &viewangles, float frametime, bool manualMovement, float forwardmove = 0, float sidemove = 0, float upmove = 0)
{
	...
	if (((in_tasstrafe.state & 1) == 0) && !manualMovement) // If we're not strafing, we still need to handle setpitch and setyaw, before the call to AdjustAngles.
	{
		if (tas_setpitch.set)
		{
	...
	if (!((in_tasstrafe.state & 1) ^ manualMovement)) // Only one should be on.
		return;
...
void DLLEXPORT CL_CreateMove ( float frametime, struct usercmd_s *cmd, int active )
{
	...
	if ( active )
	{
		...
		TAS_DoStuff(viewangles, frametime, false); // YaLTeR - do everything. The tas_strafe pass.
...
void InitInput (void)
{
	...
	gEngfuncs.pfnRegisterVariable( "tas_strafe_point_x",  "0",   0 ); // X coordinate of a point for the pointstrafe.
	gEngfuncs.pfnRegisterVariable( "tas_strafe_point_y",  "0",   0 ); // Y coordinate of a point for the pointstrafe.
