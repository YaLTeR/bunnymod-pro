...
extern cvar_t *hud_health_reqhev;
...
int CHudHealth::Draw(float flTime)
{
	...
	// YaLTeR
	if (gHUD.m_iWeaponBits & (1<<(WEAPON_SUIT)) || !hud_health_reqhev->value)
	{
		...
		{
			sscanf( hud_health_pos->string, "%i %i", &x_offset, &y_offset );
			x += x_offset;
		}
		...
		{
			SPR_DrawAdditive(0, x_temp, y_offset > 100 ? ScreenHeight : (y_offset * ScreenHeight) / 100, &gHUD.GetSpriteRect(m_HUD_cross));
		}
		else
		{
			SPR_DrawAdditive(0, x, y - y_offset, &gHUD.GetSpriteRect(m_HUD_cross));
		}

		x = CrossWidth + HealthWidth / 2 + x_offset;
		x_temp += CrossWidth / 2 + HealthWidth / 2;

		if ( hud_pos_percent->value )
		{
			x_temp = gHUD.DrawHudNumber(x_temp, y_offset > 100 ? ScreenHeight : (y_offset * ScreenHeight) / 100, DHN_3DIGITS | DHN_DRAWZERO, m_iHealth, r, g, b);
		}
		else
		{
			x = gHUD.DrawHudNumber(x, y - y_offset, DHN_3DIGITS | DHN_DRAWZERO, m_iHealth, r, g, b);
		}

		x += HealthWidth/2;
		x_temp += HealthWidth / 2;
		...
		if ( hud_pos_percent->value )
		{
			FillRGBA(x_temp, y_offset > 100 ? ScreenHeight : (y_offset * ScreenHeight) / 100, iWidth, iHeight, 255, 160, 0, a);
		}
		else
		{
			FillRGBA(x, y - y_offset, iWidth, iHeight, r, g, b, a);
		}
	}
