...
extern cvar_t sv_trigger_hurt_color; // YaLTeR

void CTriggerHurt :: Think( void )
{
	pev->nextthink = gpGlobals->time + 0.5;
	
	if ( CVAR_GET_FLOAT( "sv_trigger_hurt_display" ) == 1 )
	{
		int alpha = 128;
		int r = 255, g = 0, b = 0;
		
		alpha = CVAR_GET_FLOAT( "sv_trigger_alpha" );		
		if ( alpha > 255 )	{ alpha = 255; }
		if ( alpha < 0 )	{ alpha = 0; }
		
		if ( sv_trigger_hurt_color.string )
		{
			sscanf( sv_trigger_hurt_color.string, "%d%d%d", &r, &g, &b );			
			if ( r > 255 )	{ r = 255; }
			if ( r < 0 )	{ r = 0; }			
			if ( g > 255 )	{ g = 255; }
			if ( g < 0 )	{ g = 0; }			
...
// YaLTeR
extern cvar_t sv_trigger_multiple_color;
...
//	else
		...
		// YaLTeR
		if ( !strcmp( STRING( pev->classname ), "trigger_multiple" ) && CVAR_GET_FLOAT( "sv_trigger_multiple_display" ) == 1 )
		{
			int alpha = 128;
			int r = 255, g = 128, b = 0;
			...
			if ( sv_trigger_multiple_color.string )
			{
				sscanf( sv_trigger_multiple_color.string, "%d%d%d", &r, &g, &b );			
				if ( r > 255 )	{ r = 255; }
				if ( r < 0 )	{ r = 0; }			
				if ( g > 255 )	{ g = 255; }
				if ( g < 0 )	{ g = 0; }			
...
// YaLTeR
extern cvar_t sv_trigger_once_color;
...
void CTriggerOnce::Spawn( void )
{
	...
	// YaLTeR
	if ( CVAR_GET_FLOAT( "sv_trigger_once_display" ) == 1 )
	{
		int alpha = 128;
		int r = 255, g = 128, b = 0;
		...
		if ( sv_trigger_once_color.string )
		{
			sscanf( sv_trigger_once_color.string, "%d%d%d", &r, &g, &b );			
			if ( r > 255 )	{ r = 255; }
			if ( r < 0 )	{ r = 0; }			
			if ( g > 255 )	{ g = 255; }
			if ( g < 0 )	{ g = 0; }			
...
// YaLTeR
extern cvar_t sv_trigger_transition_color;

void CTriggerVolume :: Think ( void )
{
	pev->nextthink = gpGlobals->time + 0.5;
	
	if ( CVAR_GET_FLOAT( "sv_trigger_transition_display" ) == 1 )
	{
		int alpha = 128;
		int r = 0, g = 255, b = 255;
		...
		if ( sv_trigger_transition_color.string )
		{
			sscanf( sv_trigger_transition_color.string, "%d%d%d", &r, &g, &b );			
			if ( r > 255 )	{ r = 255; }
			if ( r < 0 )	{ r = 0; }			
			if ( g > 255 )	{ g = 255; }
			if ( g < 0 )	{ g = 0; }			
...
// YaLTeR
extern cvar_t sv_trigger_changelevel_color;

void CChangeLevel :: Think ( void )
{
	pev->nextthink = gpGlobals->time + 0.5;
	
	if ( CVAR_GET_FLOAT( "sv_trigger_changelevel_display" ) == 1 )
	{
		int alpha = 128;
		int r = 0, g = 255, b = 0;
		...
		if ( sv_trigger_changelevel_color.string )
		{
			sscanf( sv_trigger_changelevel_color.string, "%d%d%d", &r, &g, &b );			
			if ( r > 255 )	{ r = 255; }
			if ( r < 0 )	{ r = 0; }			
			if ( g > 255 )	{ g = 255; }
			if ( g < 0 )	{ g = 0; }			
...
// YaLTeR
extern cvar_t sv_trigger_push_color;

void CTriggerPush :: Think( void )
{
	pev->nextthink = gpGlobals->time + 0.5;
	
	if ( CVAR_GET_FLOAT( "sv_trigger_push_display" ) == 1 )
	{
		int alpha = 128;
		int r = 0, g = 0, b = 255;
		...
		if ( sv_trigger_push_color.string )
		{
			sscanf( sv_trigger_push_color.string, "%d%d%d", &r, &g, &b );			
			if ( r > 255 )	{ r = 255; }
			if ( r < 0 )	{ r = 0; }			
			if ( g > 255 )	{ g = 255; }
			if ( g < 0 )	{ g = 0; }			
...
// YaLTeR
extern cvar_t sv_trigger_teleport_color;
...
void CTriggerTeleport :: Spawn( void )
{
	...
	// YaLTeR
	if ( CVAR_GET_FLOAT( "sv_trigger_teleport_display" ) == 1 )
	{
		int alpha = 128;
		int r = 255, g = 128, b = 0;
		...
		if ( sv_trigger_teleport_color.string )
		{
			sscanf( sv_trigger_teleport_color.string, "%d%d%d", &r, &g, &b );			
			if ( r > 255 )	{ r = 255; }
			if ( r < 0 )	{ r = 0; }			
			if ( g > 255 )	{ g = 255; }
			if ( g < 0 )	{ g = 0; }			
