...
TYPEDESCRIPTION	CBasePlayer::m_playerSaveData[] =
{
	...
	DEFINE_FIELD( CBasePlayer, m_flTimeBeforeLoad, FIELD_FLOAT ),
...
/*
	MrBozo:
	!gLoadingGame here means that this is a level transition,
	but you can't save during a transition: you have to wait
	til the loading is all done to be able to perform a "save".
	But I don't know where we can tell when this loading is done.

	Sample code:

	extern cvar_t autosave;
	if ( autosave.value == 1 && !gLoadingGame )
	{
		CLIENT_COMMAND( edict(), "stop\n" );      // works
		CLIENT_COMMAND( edict(), "save test\n" ); // does NOT work here
	}
...
void CBasePlayer::SelectItem(const char *pstr)
{
	...
	if (m_pLastItem->m_iId == WEAPON_GAUSS)
	{
		CLIENT_COMMAND(edict(), "hud_gaussboost_disable\n");
	}
}
