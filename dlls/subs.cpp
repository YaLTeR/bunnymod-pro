...
// YaLTeR
extern cvar_t sv_info_landmark_model;
...
void CPointEntity :: Spawn( void )
{
	...
	// YaLTeR
	if ( !strcmp( STRING( pev->classname ), "info_landmark" ) )
	{
		if ( CVAR_GET_FLOAT( "sv_info_landmark_display" ) == 1 )
		{
			pev->effects = 0;
		
			if ( sv_info_landmark_model.string )
			{
				SET_MODEL( ENT(pev), sv_info_landmark_model.string );
			}
			else
			{
